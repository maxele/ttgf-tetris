		...
		begin : sv2v_autoblock_1
			...
			for (i = 0; i < 2; i = i + 1)
				begin
					...
					if ((keys_registers_p[i * 6+:6] == 3) || (keys_registers_p[i * 6+:6] == 5)) begin
...
module tt_um_maxele (
	...
	input rst_n;
	reg [199:0] field;
	reg [15:0] active_piece;
	reg signed [4:0] active_piece_x;
	reg signed [4:0] active_piece_y;
	wire [7:0] color_out;
	wire new_frame;
	reg [7:0] number_n;
	reg [7:0] number_p;
	...
	always @(*) begin
		...
		number_n = number_p;
		if (new_frame)
			number_n = -1 * (!transformation_valid && (transformation_type == 3'd4));
...
module valid_placement (
	...
	always @(*) begin
		...
		case (transformation_type_in)
			...
			default: valid = 0;
		endcase
		valid = valid && (((&(~(field_in[(y + 0) * 10+:10] & ({piece[0+:4], 6'b000000} >> x))) & (&(~(field_in[(y + 1) * 10+:10] & ({piece[4+:4], 6'b000000} >> x))))) & (&(~(field_in[(y + 2) * 10+:10] & ({piece[8+:4], 6'b000000} >> x))))) & (&(~(field_in[(y + 3) * 10+:10] & ({piece[12+:4], 6'b000000} >> x)))));
	end
...
module vga_graphics (
	...
	always @(*) begin
		...
		if (y_p < 10'd33)
			...
			lx = {x_p - 10'd48}[9:4];
			ly = {((10'd525 - 10'd10) - 10'd2) - y_p}[9:4];
